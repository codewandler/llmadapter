...
type agenticCodingCandidate struct {
	...
	cacheProviderRouting  json.RawMessage
...
func TestUseCaseAgenticCoding(t *testing.T) {
	...
	if path := os.Getenv("LLMADAPTER_COMPAT_ARTIFACT"); path != "" {
		t.Cleanup(func() {
			if t.Failed() {
				return
			}
			rowsMu.Lock()
			copiedRows := append([]compatibility.Row(nil), rows...)
			rowsMu.Unlock()
			sort.Slice(copiedRows, func(i, j int) bool { return copiedRows[i].Candidate < copiedRows[j].Candidate })
			artifact, err := compatibility.NewArtifact(
				...
				copiedRows,
				time.Since(startedAll).Seconds(),
				[]string{
					"Generated by TestUseCaseAgenticCoding when LLMADAPTER_COMPAT_ARTIFACT is set.",
					"Every approved row must report provider cache write or cache read counters.",
					"Reasoning is recorded when observable but is not required for agentic-coding approval.",
				},
			)
			if err != nil {
				t.Fatalf("build compatibility artifact: %v", err)
			}
			if err := compatibility.SaveArtifact(path, artifact); err != nil {
				t.Fatalf("write compatibility artifact: %v", err)
			}
		})
	}
...
func agenticCodingArtifactRow(candidate agenticCodingCandidate, result agenticCodingResult) compatibility.Row {
	status := compatibility.StatusApproved
	requiredStatus := "passed"
	if result.Text != "live" || result.Tools != "live" || result.StructuredOutput != "live" || result.PromptCaching != "live" || result.CacheAccounting != "live" {
...
func candidateNativeModel(candidate agenticCodingCandidate) string {
	if fromEnv := os.Getenv(candidate.modelEnv); fromEnv != "" {
		return fromEnv
	}
	return candidate.nativeModel
}
...
func candidateFamily(candidate agenticCodingCandidate) adapt.ApiFamily {
	...
	case adapt.ApiOpenAIResponses:
		return adapt.FamilyOpenAIResponses
	case adapt.ApiOpenAIChatCompletions:
		return adapt.FamilyOpenAIChatCompletions
	default:
		return adapt.ApiFamily(candidate.sourceAPI)
	}
}
...
func agenticCodingCandidates() []agenticCodingCandidate {
	return []agenticCodingCandidate{
		...
		openRouterResponsesAgenticCandidate("openrouter_qwen3_coder", "qwen3-coder", "qwen/qwen3-coder", "OPENROUTER_AGENTIC_QWEN3_CODER_MODEL"),
		qwenNextAgenticCandidate(),
...
func opus47AgenticCandidate(candidate agenticCodingCandidate, modelDBModel string) agenticCodingCandidate {
	candidate.modelDBModel = modelDBModel
	candidate.skipReasoningText = true
	candidate.adaptiveReasoning = true
	return candidate
}
...
func qwenNextAgenticCandidate() agenticCodingCandidate {
	candidate := openRouterResponsesAgenticCandidate("openrouter_qwen3_coder_next", "qwen3-coder-next", "qwen/qwen3-coder-next", "OPENROUTER_AGENTIC_QWEN3_CODER_NEXT_MODEL")
	candidate.cacheProviderRouting = json.RawMessage(`{"order":["parasail"],"allow_fallbacks":false}`)
	return candidate
}

func kimiAgenticCandidate() agenticCodingCandidate {
	candidate := openRouterResponsesAgenticCandidate("openrouter_kimi_k2_6", "kimi-k2.6", "moonshotai/kimi-k2.6", "OPENROUTER_AGENTIC_KIMI_MODEL")
	candidate.cacheProviderRouting = json.RawMessage(`{"order":["deepinfra"],"allow_fallbacks":false}`)
	return candidate
}
...
func newAgenticCodingClient(t *testing.T, candidate agenticCodingCandidate) (unified.Client, string) {
	...
	route := adapterconfig.RouteConfig{
	...
	if candidate.modelDBModel != "" {
		route.ModelDBModel = candidate.modelDBModel
	}
	cfg := adapterconfig.Config{
		...
		Routes: []adapterconfig.RouteConfig{route},
	}
...
func checkAgenticReasoning(t *testing.T, client unified.Client, model string, candidate agenticCodingCandidate) string {
	...
	if !candidate.adaptiveReasoning {
		budget := 1024
		maxReasoningTokens = &budget
	}
	resp, err := collectSmokeResponse(ctx, client, unified.Request{
		Model:           model,
		MaxOutputTokens: &maxTokens,
		Reasoning:       &unified.ReasoningConfig{Effort: unified.ReasoningEffortHigh, MaxTokens: maxReasoningTokens, Expose: true},
		Messages: []unified.Message{{
			Role:    unified.RoleUser,
	...
	if err != nil {
		t.Logf("reasoning request unsupported: %v", err)
		return string(compatibility.EvidenceUnsupported)
	}
	if !strings.Contains(strings.ToLower(responseText(resp)), "agentic reasoning ok") {
		t.Logf("reasoning response did not match marker: %q", responseText(resp))
		return string(compatibility.EvidenceUnsupported)
	}
	if !candidate.skipReasoningText && responseReasoningText(resp) == "" && resp.Usage.ReasoningTokens() == 0 {
		t.Logf("missing reasoning evidence: model=%s input_tokens=%d output_tokens=%d reasoning_tokens=%d", resp.Model, resp.Usage.InputTokens(), resp.Usage.OutputTokens(), resp.Usage.ReasoningTokens())
		return string(compatibility.EvidenceUnsupported)
	}
	return "live"
}
...
func checkAgenticPromptCache(t *testing.T, client unified.Client, model string, candidate agenticCodingCandidate) (string, string) {
	...
	if len(candidate.cacheProviderRouting) > 0 {
		if err := unified.SetOpenRouterExtensions(&req.Extensions, unified.OpenRouterExtensions{Provider: candidate.cacheProviderRouting}); err != nil {
			t.Fatalf("set cache provider routing extension: %v", err)
		}
	}
