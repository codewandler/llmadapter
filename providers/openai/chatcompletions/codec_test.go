...
func TestClientDefaultTransportRetriesTransient503(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("data: {\"id\":\"chatcmpl\",\"model\":\"gpt-test\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\"}}]}\n\n"))
		_, _ = w.Write([]byte("data: {\"id\":\"chatcmpl\",\"model\":\"gpt-test\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"ok\"},\"finish_reason\":\"stop\"}]}\n\n"))
...
func TestEncodeParallelToolCallsDefaultsFalse(t *testing.T) {
	wire, _, err := encodeRequest(unified.Request{Model: "gpt-test"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestEncodeParallelToolCallsExplicitTrue(t *testing.T) {
	enabled := true
	wire, _, err := encodeRequest(unified.Request{Model: "gpt-test", ParallelToolCalls: &enabled})
	if err != nil {
		t.Fatal(err)
	}
	if wire.ParallelToolCalls == nil || !*wire.ParallelToolCalls {
