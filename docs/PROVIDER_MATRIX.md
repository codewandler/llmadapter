...
On 2026-04-26, the full live command above was run with local credentials available for all v1 provider endpoints. Text, tools, tool continuation, gateway routing, reasoning where advertised, parallel tools where advertised, Responses continuation, invalid credentials, and invalid model normalization passed. Prompt-cache accounting passed for `anthropic`, `claude`, and `codex_responses`; MiniMax Messages cache controls remain mapped but are not marked live accounting-verified because the provider response did not report cache write/read counters in the release-candidate run.
