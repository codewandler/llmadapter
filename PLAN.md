...
Use-case compatibility live slice: `tests/e2e/TestUseCaseAgenticCoding` verifies promoted OpenAI, Codex, OpenRouter, Claude, Anthropic, MiniMax, Bedrock Converse, and selected non-Anthropic OpenRouter Responses rows for streaming text, tools, tool continuation, structured output, reasoning, prompt caching, and usage; `docs/compatibility/agentic_coding.json` records the latest live result and includes explicit Opus 4.7 rows alongside retained Opus 4.6 evidence. Current non-Anthropic approved OpenRouter rows include Moonshot Kimi K2.6, Z.AI GLM 4.6/4.7, and DeepSeek V3.2; Qwen Coder rows were live-probed but withheld because they did not expose reasoning evidence under the strict agentic-coding contract.
