...
Modeldb parameter metadata slice: llmadapter now consumes modeldb v0.14.3; resolved modeldb offering exposure metadata flows through router model resolution into provider requests, including parameter values, parameter mappings, parameter value mappings, and reasoning modes/efforts; direct Anthropic-family clients attach exact native-model metadata from the built-in modeldb catalog before encoding; OpenRouter Messages direct clients attach OpenRouter-native Anthropic Messages metadata for `anthropic/...` offerings; Anthropic adaptive effort selection uses that metadata instead of static Claude model-name matching; canonical `ReasoningEffortMax` maps to provider wire values such as Opus 4.7 `xhigh` via modeldb exposure metadata; metadata-confirmed Anthropic-family JSON-schema response formats encode through `output_config.format`; metadata-confirmed top-level cache controls encode through request `cache_control`; router candidates can reject unsupported modeldb reasoning effort values; Codex provider-local `codex`/`fast`/`powerful` aliases were removed; and auto modeldb config no longer injects llmadapter-pinned built-in aliases ahead of catalog/operator aliases
