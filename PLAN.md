...
```text
Release-candidate validation before final v1.0.0:

1. Keep live provider smoke coverage green.
2. Resolve any remaining review findings that produce incorrect routing, lossy mapping, or misleading docs.
3. Cut a final release candidate only if additional behavior changes land before v1.0.0.
```
...
Prompt cache slice: canonical `CachePolicy`, `CacheKey`, and `CacheTTL` primitives are available on unified requests; `TextPart.CacheControl` hints encode through Anthropic-family system/content blocks, including OpenRouter Messages and MiniMax Messages; modeldb-confirmed Anthropic-family top-level cache controls encode as request `cache_control`; Responses-style prompt cache keys are available for OpenAI/OpenRouter Responses and Codex Responses, with Codex mapping the key into session/window headers; live prompt-cache smoke verifies provider-reported cache write/read usage for Anthropic-family entries and Codex where credentials are available
...
Use-case compatibility live slice: `tests/e2e/TestUseCaseAgenticCoding` verifies promoted OpenAI, Codex, OpenRouter, Claude, Anthropic, MiniMax, Bedrock Converse, and selected non-Anthropic OpenRouter Responses rows for streaming text, tools, tool continuation, structured output, prompt caching, usage, and cache accounting; reasoning is recorded when observable but is optional evidence for thinking-model filters. `docs/compatibility/agentic_coding.json` records the latest live result and includes explicit Opus 4.7 rows alongside retained Opus 4.6 evidence. Current non-Anthropic approved OpenRouter rows include Moonshot Kimi K2.6, Z.AI GLM 4.6/4.7, Qwen3-Coder, Qwen3-Coder-Next, and DeepSeek V3.2.
...
Compatibility artifact automation slice: `compatibility` can load/save/render evidence artifacts, live agentic-coding e2e can write the JSON artifact after parallel subtests finish when `LLMADAPTER_COMPAT_ARTIFACT` is set, and `llmadapter compatibility-record` regenerates the use-case matrix generated section from the artifact
...
Modeldb parameter metadata slice: llmadapter now consumes modeldb v0.15.0; resolved modeldb offering exposure metadata flows through router model resolution into provider requests, including parameter values, parameter mappings, parameter value mappings, reasoning modes/efforts, and offering-specific token limits such as Codex `gpt-5.5`'s 272k serving window; direct Anthropic-family clients attach exact native-model metadata from the built-in modeldb catalog before encoding; OpenRouter Messages direct clients attach OpenRouter-native Anthropic Messages metadata for `anthropic/...` offerings; Anthropic adaptive effort selection uses that metadata instead of static Claude model-name matching; canonical `ReasoningEffortMax` maps to provider wire values such as Opus 4.7 `xhigh` via modeldb exposure metadata; metadata-confirmed Anthropic-family JSON-schema and JSON-object response formats encode through `output_config.format`; metadata-confirmed top-level cache controls encode through request `cache_control`; router candidates can reject unsupported modeldb reasoning effort values; Codex provider-local `codex`/`fast`/`powerful` aliases were removed; and auto modeldb config no longer injects llmadapter-pinned built-in aliases ahead of catalog/operator aliases
...
MiniMax Messages continuation stabilization slice: Anthropic-family `ToolChoiceNone` now encodes as provider `tool_choice: {"type":"none"}`; the MiniMax Messages live e2e tool-result continuation row opts into explicit no-tool continuation, a MiniMax-specific continuation output budget, and a single-shot tool-result instruction so M2.7's interleaved-thinking Anthropic-compatible surface has room to finish instead of re-emitting tool-call markup or stopping at length
ToolChoiceNone conformance slice: OpenAI Chat-compatible providers omit `tool_choice: "none"` when no tools are encoded because direct OpenAI Chat rejects that wire shape, while still encoding `"none"` when tools are present; OpenAI Responses-compatible providers preserve `"none"` with or without tools; downstream Anthropic Messages decodes `tool_choice: {"type":"none"}` into unified `ToolChoiceNone`
Alias documentation overhaul slice: README, CLI/config/library/getting-started docs, troubleshooting docs, use-case docs, and shipped examples now use modeldb catalog names, service-qualified model IDs, or explicit operator aliases instead of removed llmadapter-owned `codex`/`fast`/`powerful` shortcuts
...
Anthropic-compatible direct metadata slice: MiniMax Messages direct clients attach MiniMax-native built-in modeldb metadata for Anthropic-compatible requests, and OpenRouter Messages direct clients attach OpenRouter-native Anthropic Messages metadata for `anthropic/...` offerings; both wrappers keep Anthropic-native Claude metadata disabled unless the service identity is actually Anthropic
Bedrock Responses first slice: `bedrock_responses` registers Amazon Bedrock Mantle's OpenAI-compatible `/v1/responses` surface as API kind `bedrock.responses` and family `openai.responses`, reusing the existing OpenAI Responses codec/stream decoder with Bedrock-specific API-key envs, AWS SDK-backed short-term bearer token generation from profile/region credentials, region-derived Mantle base URL, modeldb service identity, registry/config docs, and shared smoke coverage hooks. Live Mantle `/v1/models` and `/v1/responses` probing currently confirms `openai.gpt-oss-120b` and `openai.gpt-oss-20b` as usable through this endpoint, while other listed Mantle IDs return provider validation errors for `/v1/responses`. Native Bedrock Converse and Chat Completions remain future endpoint shapes for covering the full Bedrock model catalog.
Bedrock Messages first slice: `bedrock_messages` registers Amazon Bedrock Mantle's Anthropic-compatible `/anthropic/v1/messages` surface as API kind `bedrock.anthropic_messages` and family `anthropic.messages`, sharing the Bedrock SDK-backed bearer-token auth path with `bedrock_responses` and reusing the Anthropic Messages codec/stream decoder. Live probing currently confirms `anthropic.claude-opus-4-7` and `anthropic.claude-haiku-4-5` through Mantle Messages in `us-east-1`; `anthropic.claude-opus-4-6` is not available as that Bedrock Mantle model ID. Text, tool use, tool-result continuation, `/v1/messages` gateway, and OpenAI Chat gateway smokes are live-verified. Reasoning is not advertised yet because adaptive-thinking requests are accepted but the live stream did not expose reasoning deltas or reasoning token accounting.
Bedrock Converse first slice: `bedrock_converse` registers native AWS SDK Bedrock Runtime `ConverseStream` as API kind and family `bedrock.converse`, with SDK credential-chain auth, auto-detected mux enablement when AWS SDK credentials are retrievable, replay continuation, text/tool/reasoning mapping, prompt cache-point encoding, shared smoke hooks, docs/config registry wiring, and inference-profile resolution that preserves explicit profile prefixes or applies a region-derived `us.`, `eu.`, `apac.`, or `global.` prefix for known models. Live smokes in `us-east-1` pass for default Sonnet 4.6 text, tool use, tool-result continuation, reasoning, OpenAI Chat gateway, and Anthropic Messages gateway; the agentic-coding compatibility run passes cache-accounting checks for Haiku 4.5, Sonnet 4.6, Opus 4.6, and Opus 4.7 through Bedrock Converse. Converse requires the tool definition to be replayed when prior `toolUse`/`toolResult` blocks are replayed, so the shared e2e has an explicit continuation-tools option for that provider. Modeldb-backed fixed and dynamic routes now prefer `RuntimeAccess.ResolvedWireID` for Bedrock Converse profile IDs, using the provider-local profile map only as a direct-client/fallback path.
```
...
env GOCACHE=/tmp/go-cache go test ./providers/openrouter/messages ./providers/anthropic/messages ./modelmeta ./providers
...
env GOCACHE=/tmp/go-cache go test ./cmd/llmadapter ./adapterconfig ./providerregistry
...
env GOCACHE=/tmp/go-cache go test ./...
env GOCACHE=/tmp/go-cache go vet ./...
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run TestBedrockResponsesModelsList -count=1 -v
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestBedrock(Responses|Messages)ModelsList' -count=1 -v
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 BEDROCK_RESPONSES_PROBE_MODELS=anthropic.claude-haiku-4-5,anthropic.claude-opus-4-7,deepseek.v3.1,deepseek.v3.2,google.gemma-3-12b-it,google.gemma-3-27b-it,google.gemma-3-4b-it,minimax.minimax-m2,minimax.minimax-m2.1,minimax.minimax-m2.5,mistral.devstral-2-123b,mistral.magistral-small-2509,mistral.ministral-3-14b-instruct,mistral.ministral-3-3b-instruct,mistral.ministral-3-8b-instruct,mistral.mistral-large-3-675b-instruct,mistral.voxtral-mini-3b-2507,mistral.voxtral-small-24b-2507,moonshotai.kimi-k2-thinking,moonshotai.kimi-k2.5,nvidia.nemotron-nano-12b-v2,nvidia.nemotron-nano-3-30b,nvidia.nemotron-nano-9b-v2,nvidia.nemotron-super-3-120b,openai.gpt-oss-120b,openai.gpt-oss-20b,openai.gpt-oss-safeguard-120b,openai.gpt-oss-safeguard-20b,qwen.qwen3-235b-a22b-2507,qwen.qwen3-32b,qwen.qwen3-coder-30b-a3b-instruct,qwen.qwen3-coder-480b-a35b-instruct,qwen.qwen3-coder-next,qwen.qwen3-next-80b-a3b-instruct,qwen.qwen3-vl-235b-a22b-instruct,writer.palmyra-vision-7b,zai.glm-4.6,zai.glm-4.7,zai.glm-4.7-flash,zai.glm-5 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run TestBedrockResponsesProbeModels -count=1 -v
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 BEDROCK_MESSAGES_PROBE_MODELS=anthropic.claude-opus-4-7,anthropic.claude-haiku-4-5,anthropic.claude-opus-4-6 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run TestBedrockMessagesProbeModels -count=1 -v
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestSmoke(TextStream|ToolUse|ToolResultContinuation)/bedrock_messages$' -count=1 -v
env AWS_PROFILE=<profile> AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestAnthropicMessagesGatewaySmoke(NonStreaming|Streaming)/bedrock_messages$|TestGatewaySmoke(NonStreaming|Streaming)/bedrock_messages$' -count=1 -v
env GOCACHE=/tmp/go-cache go test ./providers/bedrock/converse ./providerregistry ./adapterconfig ./tests/e2e
env AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestSmokeTextStream/bedrock_converse$' -count=1 -v
env AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestSmoke(ToolUse|ToolResultContinuation|ReasoningStream)/bedrock_converse$|TestGatewaySmoke(NonStreaming|Streaming)/bedrock_converse$|TestAnthropicMessagesGatewaySmoke(NonStreaming|Streaming)/bedrock_converse$' -count=1 -v
env AWS_REGION=us-east-1 GOCACHE=/tmp/go-cache TEST_INTEGRATION=1 go test ./tests/e2e -run 'TestSmokeTextStream|TestSmokeToolUse|TestSmokeToolResultContinuation|TestGatewaySmoke' -count=1 -v
env GOCACHE=/tmp/go-cache GOMODCACHE=/tmp/go-mod-cache go build ./...
